...
func _on_NPC_body_exited(body):
	if body.is_in_group("player"):
		...
		$Sprite_tecla.visible = false
		if(cerrarTienda):
			GLOBAL.emit_signal("HUDSelectorShipInvisible")
			cerrarTienda=false
...
func _input(event):
	if event.is_action_pressed("ui_select") and $Sprite_tecla.visible == true:
			...
			var dialogoNPC = Dialogic.start("NPC_Mecanico")
			dialogoNPC.pause_mode = Node.PAUSE_MODE_PROCESS
			dialogoNPC.connect("timeline_end", self, "unpause")
			add_child(dialogoNPC)
			##emitir una señal para poder ver la tienda.
			

func unpause(timeline_name):
	get_tree().paused = false
	##esta señal me mostrará la tienda
	GLOBAL.emit_signal("HUDSelectorShip")
	cerrarTienda=true
